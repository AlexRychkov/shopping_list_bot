...
class ListItemController(private val listItemRepository: ListItemRepository) extends Controller {
  ...
  override val controller: HttpRoutes[IO] = HttpRoutes.of[IO] {
    ...
    case GET -> Root / LongVar(listId) / "item" => Ok(listItemRepository.get(listId))

    case GET -> Root / LongVar(listId) / "item" / LongVar(itemId) => Ok(listItemRepository.get(listId, itemId))

    case req@POST -> Root / LongVar(listId) / "item" => for {
    ...
    case req@PATCH -> Root / LongVar(listId) / "item" / LongVar(itemId) / "mark" => for {
      mark <- req.as[ItemMark]
      resp <- Ok(listItemRepository.setMark(listId, itemId, mark))
    } yield resp

    case req@PATCH -> Root / LongVar(listId) / "item" / LongVar(itemId) / "comment" => for {
      comment <- req.as[String]
      resp <- Ok(listItemRepository.setComment(listId, itemId, comment))
    } yield resp

    case req@PATCH -> Root / LongVar(listId) / "item" / LongVar(itemId) / "price" => for {
      price <- req.as[BigDecimal]
      resp <- Ok(listItemRepository.setPrice(listId, itemId, price))
    } yield resp

    case DELETE -> Root / LongVar(listId) / "item" / LongVar(itemId) =>
